...
class FunASRNano(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(f"rank: {rank}, 模型构建完成。")
    ...
    def data_load_speech(
        ...
        )  # [: self.max_token_length] 限制最大token长度
        attention_mask = torch.tensor([1] * len(input_ids), dtype=torch.int32)
        labels = torch.tensor(labels, dtype=torch.int64)  # [: self.max_token_length] 限制最大token长度
    ...
    def inference_prepare(
        ...
        **kwargs,
        ...
        if kwargs.get("batch_size", 1) > 1:
            raise NotImplementedError("批量解码尚未实现")
